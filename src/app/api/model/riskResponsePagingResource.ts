/**
 * API
 * No description provided (generated by Swagger Codegen https://github.com/swagger-api/swagger-codegen)
 *
 * OpenAPI spec version: 0.1
 * 
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git
 * Do not edit the class manually.
 */
import { Type, Expose } from 'class-transformer';
// @dynamic

import { Meta } from './meta';
import { RiskResponse } from './riskResponse';

import { Observable }                                        from 'rxjs';
import { AbstractControl, FormControl, FormGroup, FormArray, Validators }                            from '@angular/forms';

export class RiskResponsePagingResource { 


    /**
     * Data wrapper
     */
    @Expose()
    @Type(() => RiskResponse)
    data!: Array<RiskResponse>;
    @Expose()
    meta!: Meta;

    /**
     * Description: Data wrapper
     * Complex type: RiskResponse
     * datatype: Array&lt;RiskResponse&gt;
     * datatypeWithEnum: Array&lt;RiskResponse&gt;
     * data: Array<RiskResponse>   
     */
    /**
     * datatype: Meta
     * datatypeWithEnum: Meta
     * meta: Meta   
     */

    // validations?: Map<string, Array<{[key: string]: string}>> = new Map<string, Array<{[key: string]: string}>>();

    constructor(init: Partial<RiskResponsePagingResource> = {}) {
         
                            this.data = init.data?.map(x => new RiskResponse(x)) || [] ,
                        this.meta = new Meta(init.meta || {})
    }

    static   getForm(data?: RiskResponsePagingResource | RiskResponsePagingResource[] | null): FormGroup {


        if(!data) {
            return RiskResponsePagingResource.getFormGroup(new RiskResponsePagingResource());
        }

         if(Array.isArray(data)) {
            let arrayForm = new FormArray<any>([]);
            
            if(data.length > 0) {
                arrayForm =  new FormArray<any>(data.map(item => RiskResponsePagingResource.getFormGroup(item)));                
            }
            return new FormGroup({
                arrayForm
            });        
        } else {
             return  RiskResponsePagingResource.getFormGroup(data);
        }       
     
    }

     /**
   * Get FormGroup
   *
   * @param {object} data
   * @returns {FormGroup}
   */
  static   getFormGroup(data?: RiskResponsePagingResource): FormGroup {
       
        return new FormGroup({           
                        meta: (() => { 
                            const fg = Meta.getForm(data?.meta);
                            fg.addValidators([]);
                            return fg;
                        })()
        });
    }
  

}